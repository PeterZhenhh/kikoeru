...
const all = async (
    ...
    switch (clientSP.searchType) {
        case "circle":
            url.pathname += encodeURIComponent(
                `$circle:${clientSP.searchKeyword}$/`,
            );
            break;
        case "va":
            url.pathname += encodeURIComponent(
                `$va:${clientSP.searchKeyword}$/`,
            );
            break;
        case "tag":
            url.pathname += encodeURIComponent(
                `$tag:${clientSP.searchKeyword}$/`,
            );
            break;
        case "keyword":
    ...
    let data;
    let ret = { totalCount: 0, jFullNums: [] } as {
    ...
    ret.jFullNums = (data.works ?? []).map((item: { source_id: string }) =>
